...
Dialog.addCheckbox("DO NOT compute particles, use ROI manager data instead", useROI);
...
for (index=0;index<numROIs;index++) {
	...
	if (localback) { // if necessary, compute a local base
	...
	triang_volume=triangVolume(xs, ys, widths, heights,xlengthconversion,ylengthconversion,base,negheights);
	...
	Table.set("radius from triang. vol.", rowIndex, triangvolRadius,myTable);
...
function triangVolume(xs, ys, widths, heights,pw,ph,base,negh) { // everything is assumed to be in nm here
	result=0;
	numelements=0;
