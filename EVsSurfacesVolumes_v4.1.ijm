...
Dialog.addMessage("------- Z units must be nm ------", 14, "blue");
Dialog.setInsets(0,0,0);
...
if (identify=="Threshold") {  // if we want to compute new ROIs
	...
	if (!idbil) { // no bilayers separate identification
		...
		ROIscanning(RoiManager.size-1,myTable,localback,base,basemark,areaconversion,xlengthconversion,ylengthconversion,negheights,Npoints,idbil,bilmin,bilmax,bilstd,bilcorrection);
	}
}
