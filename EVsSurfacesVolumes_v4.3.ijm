...
Dialog.create("EVs radius evaluation v."+ver);
...
Dialog.addNumber("min. bilayer diameter", bilmindiam, 1, 6, "nm");
Dialog.addCheckbox("use correction for bilayer surface computation", bilcorrection);
...
Dialog.addToSameRow();
//Dialog.addCheckbox("allow negative heights (holes)", negheights); // check line 108 if uncomment this
...
if (identify=="Use ROIs") { // if we want to use ROIs in ROI manager
	if (RoiManager.size>0){
		ROIscanning(0,myTable,localback,base,basemark,areaconversion,xlengthconversion,ylengthconversion,negheights,Npoints,idbil,bilmin,bilmax,bilstd,bilcorrection);
	} else {
		exit("ERROR! - NO ROIs!");
	}
}
...
if (identify=="Threshold") {  // if we want to compute new ROIs
	...
	if (!idbil) { // no bilayers separate identification
		...
		ROIscanning(0,myTable,localback,base,basemark,areaconversion,xlengthconversion,ylengthconversion,negheights,Npoints,idbil,bilmin,bilmax,bilstd,bilcorrection);
	} else { // try to identify bilayers and put them at the end of the table
		// first particles
		...
		ROIscanning(0,myTable,localback,base,basemark,areaconversion,xlengthconversion,ylengthconversion,negheights,Npoints,idbil,bilmin,bilmax,bilstd,bilcorrection);
		ROIsparticles=RoiManager.size-1;
		// then bilayers
		...
		ROIscanning(ROIsparticles,myTable,localback,base,basemark,areaconversion,xlengthconversion,ylengthconversion,negheights,Npoints,idbil,bilmin,bilmax,bilstd,bilcorrection);
	}
}
...
// ++++++++++++++++++++++++++++++++++++ ROIs scanning +++++++++++++++++++++++++++++++++++
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function ROIscanning(startROI,myTable,localback,base,basemark,areaconversion,xlengthconversion,ylengthconversion,negheights,Npoints,idbil,bilmin,bilmax,bilstd,bilcorrection) {
	...
	for (index=startROI;index<numROIs;index++) {
		...
		if (idbil && mean>=bilmin && mean<=bilmax && std<=bilstd && bilcorrection) { // if is a bilayer and area correction is true
			surf_total=triang_area; // use only the top surface area
		}
