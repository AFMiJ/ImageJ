...
if (identify=="Threshold") {  // if we want to compute new ROIs
	...
	if (!idbil) { // no bilayers separate identification
		...
		ROIscanning(ROInextsparticle,myTable,localback,base,basemark,areaconversion,xlengthconversion,ylengthconversion,negheights,Npoints,idbil,bilmin,bilmax,bilstd,bilcorrection);
	}
	resetThreshold;
}
